...
class ExternalTableAttribute extends Attribute {
    ...
    function delete()
    {
        if( $this->values['content_key'] )
        {
            ...
            $query .=   "`content_key` = ".$this->module->wc->db->escape_string($this->values['content_key']);
            
            if( !$this->module->wc->db->deleteQuery($query) ){
                return false;
            }
        }
    ...
    protected function globalInsert( $content_key, $externalTableFields )
    {
        ...
        foreach( $externalTableFields as $valueArray )
        {
            ...
            foreach( $valueArray as $column => $value ){
                $query .=   ", `".$this->module->wc->db->escape_string($column)."` ";
            }
            
            $query .=   " ) VALUES ('".$this->module->wc->db->escape_string($content_key)."'";
            
            foreach( $valueArray as $column => $value ){
                $query .=   ", '".$this->module->wc->db->escape_string($value)."' ";
            }
            
            $query .=   " ) ";
            
            $id = $this->module->wc->db->insertQuery($query);
